...
const projectDefinitions: Project[] = [
  {
    ...
    findings: [
      '第一版随机森林的测试集R²为借出量0.644、还入量0.436；梯度提升树分别为0.142和0.139。在当次字段、参数与随机划分条件下，随机森林的两项结果均更高。R²用于衡量预测对实际数量变化的解释程度，不能直接当作准确率。',
      '第一版随机森林的借出量RMSE约为3.04，还入量约为3.85。RMSE是预测误差平方平均后再开方的结果，单位为次／站点小时；这两项数值记录了原实验中的误差水平。',
      '第一版形成了按车辆与车位供需缺口计算价格倍率、建议价格和理由的策略结果，并通过看板、接口及答辩展示呈现需求与定价的使用方式。价格建议属于规则模拟，尚无真实运营收益验证。',
      '后续修复删除三个同小时统计字段，改用时间切分，并完成2.4万行三组冒烟测试；该结果用于核对字段与切分的影响，尚未完成全量正式重训。',
    ],
    boundaries: [
  ...
  },
  {
    ...
    story: '数据来自Microsoft官方发布的AdventureWorksDW2022虚构样例数据库，下载数据库备份文件后，在本地SQL Server中还原，再读取网络直销与经销商销售表及相关维表。总览页显示约1.10亿美元收入和11.43%的估算毛利率，这些数字本身没有告诉我好或坏，因为没有目标，也没有合适的对照。继续按渠道拆分以后，Reseller贡献了大部分收入，估算毛利贡献却很低；问题这才从“整体表现如何”缩小到渠道、品类和SKU。',
    ...
    aiCollaboration: [
      ...
      { title: '哪些结果由我核对', text: '我使用Power Query检查整表质量，比较SQL与Power BI总额，逐页检查筛选器和指标是否联动，再根据渠道、品类、SKU和地区结果修正分析结论。' },
  ...
  },
  {
    ...
    aiCollaboration: [
      ...
      { title: 'AI怎样参与', text: 'AI主要负责读取代码库、实现FastAPI、SQLite、DuckDB、Jinja2页面、测试和文档并参与产品定位、使用流程和结果解释。我负责确定产品服务谁、哪些阶段保留、每种状态代表什么，以及哪些结果允许导出。' },
  ...
  },
  {
    ...
    aiCollaboration: [
      ...
      { title: 'AI负责哪些实现工作', text: '我负责给ai提供高德api，ai负责请求、缓存、失败重试、数据清理、NetworkX网络构建、Folium地图和HTML展示代码，并参与技术路线与结果解释。' },
  ...
  },
  {
    ...
    shift: '本项目完成的是数据处理。我先统一表头、证券代码和年份，再以研发数据为左表，按“证券代码＋年份”左连接专利数据：保留研发表的全部记录，匹配成功时补充专利字段，未匹配时保留缺失值。合并后检查匹配情况与记录数量，输出供后续研究使用的表格；本项目没有开展研发投入与专利产出之间的数据分析。',
    ...
    findings: [
      '统一不同文件的表头、证券代码与年份，并将各年度专利数据纵向合并。',
      '以研发数据为左表，按证券代码和年份补充专利字段，保留全部研发记录及未匹配状态。',
      '输出合并表与匹配统计，完成后续分析所需的数据准备；本项目未开展统计分析或因果分析。',
    ],
    boundaries: [
